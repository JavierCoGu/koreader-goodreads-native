...
firmware 5.18.2), or `missing`. The helper picks its transport from that:

- `available`: attach the agent described below.
- `cvm`: post through the framework's library service with `lipc-hash-prop
  com.lab126.readnow kppGoodReads` (see below).
- `missing`: publish `failed_stage=runtime_missing` immediately rather than
  exiting silently into KOReader's 30-second poll.

KOReader makes the same check once per session. With no runtime it does not
queue the helper at all, so periodic and close checkpoints do not retry an
impossible transport. Annotation helpers still need attach; on `cvm` they
publish `runtime_unsupported` as a retryable result, which preserves the
durable outbox for a later firmware.

### Library-service transport (cvm firmware)

`GoodReadsHandler` in the framework's `EInkReadNowService.jar` is exported as
the `kppGoodReads` hash-array property on `com.lab126.readnow`. For
`kppMycdActivityName = "PostReview"` it reads `asin`, `progress`, and `note`,
builds the same `PostShareProgressRequest` (`progress_type` `Percent`) with
the note as `note_text`, sends it through `GrokService`, and writes
`result = "true"` into the returned hash only for HTTP 202. It sets no
`social_networks` and no `x-gr-*` headers. Goodreads answers HTTP 400 for an
empty or blank note, so KOReader passes a validated, user-chosen note
(default `Reading`) that appears publicly with each update. The helper bounds
the call to 30 seconds, requires `result = "true"` plus the echoed ASIN and
percentage, and only then writes the state file.
